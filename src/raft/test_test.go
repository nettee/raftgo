...
func TestReElection(t *testing.T) {
	...
	fmt.Println("Test: the leader disconnects ...")
	fmt.Printf("Set: disconnect [%d]\n", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	fmt.Println("Test: the old leader rejoins")
	fmt.Printf("Set: connect [%d]\n", leader1)
	cfg.connect(leader1)
	...
	fmt.Printf("Set: disconnect [%d] and [%d]\n", leader2, (leader2 + 1) % servers)
	...
	fmt.Println("Test: a quorum arises")
	fmt.Printf("Set: connect [%d]\n", (leader2 + 1) % servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	fmt.Println("Test: re-join of last node")
	fmt.Printf("Set: connect [%d]\n", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestFailAgree(t *testing.T) {
	...
	fmt.Printf("Test: follower [%d] disconnects\n", (leader+1) % servers)
...
func TestFailNoAgree(t *testing.T) {
	...
	leader := cfg.checkOneLeader()
	fmt.Printf("Test: follower [%d], [%d] and [%d] disconnects\n",
	...
	// repair failures
	fmt.Printf("Test: follower [%d], [%d] and [%d] reconnected\n",
		(leader + 1) % servers, (leader + 2) % servers, (leader + 3) % servers)
	cfg.connect((leader + 1) % servers)
...
func TestRejoin(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	fmt.Printf("Test: old leader [%d] disconnects\n", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	fmt.Printf("Test: new leader [%d] disconnects\n", leader2)
	cfg.disconnect(leader2)

	// old leader connected again
	fmt.Printf("Test: old leader [%d] reconnected\n", leader1)
	cfg.connect(leader1)

	cfg.one(104, 2)

	// all together now
	fmt.Printf("Test: new leader [%d] reconnected\n", leader2)
	cfg.connect(leader2)
...
func TestBackup(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	fmt.Printf("Test: disconnect followers [%d], [%d] and [%d]\n",
	...
	for i := 0; i < 50; i++ {
		...
		cfg.rafts[leader1].Start(101 + i)
	}

	time.Sleep(RaftElectionTimeout / 2)
	fmt.Println("Test: disconnect all servers")
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	// allow other partition to recover
	fmt.Printf("Test: [%d], [%d] and [%d] reconnected\n",
		(leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	// now everyone
	fmt.Println("Test: all servers reconnected")
	for i := 0; i < servers; i++ {
		cfg.connect(i)
	}
	cfg.one(rand.Int(), servers)
